...
function ReturnItem({ navigateTo, darkMode }) {
  const t = useDark(darkMode);

  const features = [
    { icon: "📋", label: "View items waiting to be returned" },
    { icon: "👤", label: "Verify owner details" },
    { icon: "✅", label: "Mark item as returned" },
  ];
    ...
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 560, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ width: 72, height: 72, borderRadius: 18, background: "linear-gradient(135deg,#EAF3DE,#D8EDCA)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", fontSize: 32 }}>🔄</div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: t.text, marginBottom: 8 }}>Return Item</h1>
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 28px", textAlign: "left" }}>
            {features.map(f => (
              <li key={f.label} style={{
                ...
                <div style={{ width: 32, height: 32, borderRadius: 8, background: "#EAF3DE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{f.icon}</div>
                {f.label}
              </li>
            ))}
          </ul>
